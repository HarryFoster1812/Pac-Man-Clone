...
class Pacman:
    ...
    def __init__(self, start_pos: list, maze: Maze) -> None:
        ...
        self.next_cell = [0, 0]
        # the current direction that pacman is traveling
        self.direction = [0, 0]
        ...
        self.image = GameImage(
            "assets/PacManRight.gif",
            calculate_rotations=True)  # need to fill this out
        self.tick_count = 0
    ...
    def tick(self):
        ...
        current_cell = self._maze_.maze[int(
    ...
    def snapPosition(self):
        ...
        match(self.direction):
            ...
            case [0, 1]: next_position[0] = self.target_position[0]
            # moving in the y direction so snap to the middle of the x cell
            case [0, -1]: next_position[0] = self.target_position[0]
            # moving in the x direction so snap to the middle of the y cell
            case [1, 0]: next_position[1] = self.target_position[1]
            # moving in the x direction so snap to the middle of the y cell
            case [-1, 0]: next_position[1] = self.target_position[1]
    ...
    def checkChangeDirection(self) -> bool:
        if self.direction != self.next_direction:
            ...
            next_cell = self._maze_.maze[int(
                current_cell_y)][int(current_cell_x)]
