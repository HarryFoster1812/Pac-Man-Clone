...
class Ghost:
    ...
    def __init__(self, start_pos, _maze_, pacman: Pacman) -> None:
        # the current state of the ghost, 0 = default, 1 = scatter, 2 = fright,
        # 3 = dead
        ...
        # the x and y co-ordinates of pacman
        self.canvas_position = [start_pos[0], start_pos[1]]
        ...
        self._white_frightened_image_ = GameImage(
            "assets/Ghosts/WhiteFrightendGhost.gif")
    ...
    def tick(self):
        ...
        if self.state == GhostState.IN_GHOST_HOUSE:
            self.canvas_position[0] += self.direction[0] * \
                Ghost.SPEED * self.speed_modifier
            self.canvas_position[1] += self.direction[1] * \
                Ghost.SPEED * self.speed_modifier
            ...
            next_cell = self._maze_.maze[int(
                self.next_cell[1])][int(self.next_cell[0])]

            if isinstance(next_cell, Wall):
                # snap ghost to the correct placement next to the wall
                self.direction = [
                    component * -1 for component in self.direction]
            ...
            # move to correct y
            if self.direction == [
                    0, -1] and self.checkMovedPassed(436, self.direction[1], self.canvas_position[1]):
            ...
            # move to correct y
            if self.direction[0] != 0 and self.checkMovedPassed(
                    self.ghost_house_target[0], self.direction[0], self.canvas_position[0]):
        ...
        if self.current_cell == self.next_cell:
            ...
            temp_current_cell = self._maze_.maze[int(
                self.current_cell[1])][int(self.current_cell[0])]

            # check if we have hit a teleport square
            if isinstance(
                    temp_current_cell,
                    Moveable) and temp_current_cell.is_teleport:
                # get the index of the other teleport square
                index_other_cell = self._maze_.getOtherTeleportSquareLocation(
                    temp_current_cell)
                # change co-ordinates
                index_next_square_after_teleport = [
                    index_other_cell[0] + self.next_direction[0],
                    index_other_cell[1] + self.next_direction[1]]
                ...
                self.calculateNextCell()
                temp_next_cell = self._maze_.maze[int(
                    self.next_cell[1])][int(self.next_cell[0])]
                # check if next cell is junction
                if isinstance(temp_next_cell,
                              Moveable) and temp_next_cell.is_juction:
    ...
    def snapPosition(self):
        ...
        match(self.direction):
            # moving in the y direction so snap to the middle of the x cell
            case [0, 1]: self.canvas_position[0] = self.target_position[0]
            # moving in the y direction so snap to the middle of the x cell
            case [0, -1]: self.canvas_position[0] = self.target_position[0]
            # moving in the x direction so snap to the middle of the y cell
            case [1, 0]: self.canvas_position[1] = self.target_position[1]
            # moving in the x direction so snap to the middle of the y cell
            case [-1, 0]: self.canvas_position[1] = self.target_position[1]
    ...
    def checkMovedPassed(
        ...
        if current_pos == target_pos:
            return False
        # is is not equal so check if component positive and the target
    ...
    def calculateNextDirection(self, junction_cell):
        ...
        decisions = self.calculateValidDecisions(
            junction_cell, self.state, self.direction)
        ...
        if self.state == GhostState.FRIGHTENED:
            ...
            for direction in decisions:
                ...
                distance = self.pythagoras(
                    self.target, junction_cell_plus_direction)
    ...
    def calculateValidDecisions(
            self,
            junction_cell: Moveable,
            ghost_state: int,
            current_direction: list):
        possible = [[1, 0], [-1, 0], [0, 1], [0, -1]]
        opposite_direction = [
            direction * -1 for direction in current_direction]
        # the ghost can not move in the opposite direction
        possible.remove(opposite_direction)

        # check if the juction allows ghost to move up
        # if it doesnt the only exception to this is if the ghost is in frightend mode
        # is we are moving down then we have already removed the possibility
        # from the list
        if not junction_cell.can_move_up and ghost_state != GhostState.FRIGHTENED and current_direction != [
                0,
                1]:
            possible.remove([0, -1])  # the ghost can not move up

        # copy the array as we are removing things from the original so it will
        # mess up indexes
        ...
        for direction in possible_copy:
            ...
            cell_next_to_juction = self._maze_.maze[junction_location[1]
                                                    ][junction_location[0]]
    ...
    def serialise(self) -> dict:
        return {
            ...
            "state": self.state._value_,
            "next_direction": self.next_direction,
            "is_dead": self.is_dead,
            "is_frightened": self.is_frightened,
            "speed_modifier": self.speed_modifier,
            "ghost_house_target": self.ghost_house_target,
            "dot_counter": self.dot_counter,
            "dot_limit": self.dot_limit,
            "colour": self.colour,
    ...
    def parse(self, info: dict):
        for key, value in info.items():
            if key == "state":
                self.state = GhostState.get_state_by_value(value)
            else:
