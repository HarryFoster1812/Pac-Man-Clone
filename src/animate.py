...
class Animate:
    ...
    1) -> None:
        ...
        # check if the gif/image has more than one frame and it is intended to
        # be used as a gif
    ...
    def update(self):
        ...
        self.loop = self.parent.after(
    self.delayMS, self.update)  # recall the loop
    ...
    def addParent(self, new_parent, x: int = 0, y: int = 0):
        ...
        elif isinstance(self.parent, Canvas):           
                self.id=self.parent.create_image(
                    x, y, image=self.fames[self.currentFrame], anchor="ne")
    ...
    def toggleAnimation(self):
        if hasattr(
    self,
     # we need to check if the loop attribute is created as for a static image
     # (1 frame) the loop will not be created
     "loop"):
            if self.is_enabled:
